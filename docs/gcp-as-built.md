...
**Where we granted more than you asked for.** In dev you hold `cloudsql.admin`, `storage.admin`, `secretmanager.admin` and `artifactregistry.admin` — three of which you explicitly listed as *not asked for*, with "instance creation is yours" and "bucket creation and lifecycle are yours" beside them. That was Trevor's call, on the reasoning that dev holds synthetic data only and the alternative is you blocked on us for every bucket. **Staging is exactly your list.** If you would rather dev matched your file too, say so and we will narrow it — your version is the more defensible one and we are not attached to ours.

**Where we granted less.** `github-deploy` actAs: you listed all four service accounts; we bound `api` and `retention` only, since the integration and backup jobs do not deploy yet. Ask and we add the other two.
...
**Your `CREATE ROLE` question** — whether you run `roles.sql` with a privileged user once and we rotate after, or we run it and you get an ordinary user — is Trevor's to answer and he has it. It is not blocking anything else in this document.
